...
p <- plotDR(sce, color_by = testparams, facet_by = "patient.id", ncol=1)
...
p <- ggplot(q, aes(x, y)) + 
  ...
  facet_wrap(q$patient.id, nrow = 1) +
  theme_cowplot()
p
...
# Get 80th percentile values for all channels 
...
get_pat <- function(s) {
  pat <-  paste0(strsplit(s, "_")[[1]][2],"_", strsplit(s, "_")[[1]][1])
  return(pat)
}

get_con <- function(s) {
  con <-  strsplit(s, "_")[[1]][1]
  return(con)
}

scdf <- scdf %>% rowwise() %>% mutate(patient = get_pat(sample_id), 
                                                    condition = get_con(sample_id))
